...
export default function SignupPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(payload?.error || "We couldn't activate your invitation");
      }
      setMessage(payload?.message || "Account ready. Please verify your email.");
      setName("");
      setEmail("");
      setPassword("");
    } catch (err: any) {
      setError(err?.message || "We couldn't activate your invitation");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Layout title="Finish invitation">
      <div className="mx-auto max-w-md space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl font-semibold text-gray-900">Finish setting up your account</h1>
          <p className="text-sm text-gray-600">
            ProjectDesk accounts are issued by invitation. Use this page only if an administrator has already approved
            your email address.
          </p>
        </div>
